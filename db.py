...
def init_db() -> None:
    with get_connection() as conn:
        conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_mp_match
                ON match_players(match_id);
            
            CREATE INDEX IF NOT EXISTS idx_mp_puuid
                ON match_players(puuid);
            
            CREATE INDEX IF NOT EXISTS idx_m_season
                ON matches(season_id);
            
            CREATE INDEX IF NOT EXISTS idx_m_map
                ON matches(map_name);
        """)
...
def ignore_season(season_id: str) -> None:
    with get_connection() as conn:
        conn.execute(
            "INSERT INTO ignored_seasons(season_id) VALUES (?) ON CONFLICT DO NOTHING",
            (season_id,),
        )
        conn.commit()
...
def unignore_season(season_id: str) -> None:
    with get_connection() as conn:
        conn.execute("DELETE FROM ignored_seasons WHERE season_id = ?", (season_id,))
        conn.commit()
...
def get_matches(season_id: Optional[str] = None, map_name: Optional[str] = None) -> list[dict]:
    ...
    params = []
    if season_id and season_id not in get_ignored_seasons():
        query += " AND season_id = ?"
...
def get_map_stats(season_id: Optional[str] = None) -> list[dict]:
    ...
    if season_id:
        ...
        query += " AND m.season_id not in (?)"
        params.append(",".join(get_ignored_seasons()))
